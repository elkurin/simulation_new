...
namespace {
	ofstream take_log_type("data_long_type_number50_test.log");
	ofstream take_log_network("data_long_network50_test.log");
	ofstream take_log_outside("data_long_outside50_test.log");
	ofstream take_log_devdev("data_long_devdev50_test.log");
	ofstream take_log_coef("data_long_coef50_test.log");
	ofstream take_log_inside("data_long_inside50_test.log");
	ofstream take_log_dev("data_long_dev50_test.log");
}
...
int main(void)
{
	...
	rep(l, run_time) {
		...
		rep(t, time_end) {
			devdev = 0;
			process(t);
			rep(i, cell_type) aver_num[i] += a[i];
			if (t % 1000 == 0) {
				rep(i, cell_type) {
					if (aver_num[i]) take_log_dev << (double)(dev[i] - prev_dev[i]) / (double)(aver_num[i]) * 1000 << " ";
					else take_log_dev = 0;
					prev_dev[i] = dev[i];
					aver_num[i] = 0;
				}
			...
			if (t <= 100000 && t % 2000 == 1000) evolve();
		}
	}
