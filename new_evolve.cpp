...
namespace {
	ofstream take_log_type("data_evolve_type_number1_large.log");
	ofstream take_log_network("data_evolve_network1_large.log");
	ofstream take_log_outside("data_evolve_outside1_large.log");
	ofstream take_log_devdev("data_evolve_devdev1_large.log");
	ofstream take_log_coef("data_evolve_coef1_large.log");
}
...
void redundancy(void)
{
	rep(i, cell_type) {
		rep(j, N) {
			if (j == 0) {
				cell[i].coef[0][1] = begin_coef.at(i).at(0);
			} else if (j % 2) {
				cell[i].coef[j - 2][j] = begin_coef.at(i).at(j - 1);
			} else if (j != N - 1) {
				cell[i].coef[j][j + 2] = begin_coef.at(i).at(j + 1);
			} else {
				cell[i].coef[N - 2][N - 1] = begin_coef.at(i).at(N - 1);
			}
		}
	}
}

void catalyst_redund_two(void)
{
	rep(i, cell_type) {
		cell[i].catalyst[0][1] = 7;
		cell[i].catalyst[1][3] = 0;
		cell[i].catalyst[3][5] = 1;
		cell[i].catalyst[5][7] = 3;
		cell[i].catalyst[7][9] = 5;
		cell[i].catalyst[0][2] = 8;
		cell[i].catalyst[2][4] = 0;
		cell[i].catalyst[4][6] = 2;
		cell[i].catalyst[6][8] = 4;
		cell[i].catalyst[8][9] = 6;
	}
}
...
void init(void)
{
	...
	rep(i, N) {
		...
		go[i] = (i % 3 / 2) * 1;
		coef_decrease[i] = 0;
		if (i == N - 1) coef_decrease[i] = 0.005;
	}
	nut_coef = 0.1;
	nut_reversible = 0;
	aver_nut = 1;
...
Cell internal(Cell p)
{
	...
	rep(i, N) {
		rep(j, N) {
			new_con[j] += time_bunkai * p.coef[i][j] * prev_con[i] * prev_con[p.catalyst[i][j]];
			new_con[i] -= time_bunkai * p.coef[i][j] * prev_con[i] * prev_con[p.catalyst[i][j]];
		}
	}
	...
	rep(i, N) {
		rep(j, N) {
			new_con[i] += time_bunkai * reversible[i][j] * prev_con[j];
			new_con[j] -= time_bunkai * reversible[i][j] * prev_con[j];
		}
	}
...
void evolve(void)
{
	...
	cell[cell_number].catalyst[0][1] = 7;
	cell[cell_number].catalyst[1][3] = 0;
	cell[cell_number].catalyst[3][5] = 1;
	cell[cell_number].catalyst[5][7] = 3;
	cell[cell_number].catalyst[7][9] = 5;
	cell[cell_number].catalyst[0][2] = 8;
	cell[cell_number].catalyst[2][4] = 0;
	cell[cell_number].catalyst[4][6] = 2;
	cell[cell_number].catalyst[6][8] = 4;
	cell[cell_number].catalyst[8][9] = 6;

	rep(j, N) {
		if (j == 0) {
			cell[cell_number].coef[0][1] = begin_coef.at(cell_number).at(0);
		} else if (j % 2) {
			cell[cell_number].coef[j - 2][j] = begin_coef.at(cell_number).at(j - 1);
		} else if (j != N - 1) {
			cell[cell_number].coef[j][j + 2] = begin_coef.at(cell_number).at(j + 1);
		} else {
			cell[cell_number].coef[N - 2][N - 1] = begin_coef.at(cell_number).at(N - 1);
		}
	}
