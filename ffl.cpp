...
namespace {
	ofstream take_log_type("data_ffl_type_number10_0.001.log");
	ofstream take_log_network("data_ffl_network10_0.001.log");
	ofstream take_log_outside("data_ffl_outside10_0.001.log");
	ofstream take_log_devdev("data_ffl_devdev10_0.001.log");
	ofstream take_log_coef("data_ffl_coef10_0.001.log");
	ofstream take_log_inside("data_ffl_inside10_0.001.log");
}
...
Cell internal(Cell p)
{
	...
	//ただのリアクション
	new_con[0] += time_bunkai * p.nut_zero_coef * prev_nut_con;
	new_nut_con -= time_bunkai * p.nut_zero_coef * prev_nut_con;
	
	new_con[1] += time_bunkai * p.coef[0][1] * prev_con[0];
	new_con[0] -= time_bunkai * p.coef[0][1] * prev_con[0];

	new_con[2] += time_bunkai * p.coef[0][2] * prev_con[0];
	new_con[0] -= time_bunkai * p.coef[0][2] * prev_con[0];

	new_con[2] += time_bunkai * p.coef[1][2] * prev_con[1];
	new_con[1] -= time_bunkai * p.coef[1][2] * prev_con[1];
