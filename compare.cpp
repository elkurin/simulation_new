...
namespace {
	ofstream take_log_type("data_per_type_number10.log");
	ofstream take_log_network("data_per_network10.log");
	ofstream take_log_outside("data_per_outside10.log");
	ofstream take_log_devdev("data_per_devdev10.log");
	ofstream take_log_coef("data_per_coef10.log");
	ofstream take_log_inside("data_per_inside10.log");
	ofstream take_log_hostdev("data_per_hostdev10_min_locked.log");
	ofstream take_log_win("data_per_win10_case2_0.4.log");
	ofstream take_log_winner("data_per_two_winner_0.01.log");
}
...
void all_init(void)
{
	rep(j, cell_type) {
		rep(i, N) {
			if (i % 3 != 2) begin_coef.at(j).at(i) = 0.4;
			else begin_coef.at(j).at(i) = 2.4;
		}
	}
}
...
Cell internal(Cell p)
{
	...
	//細胞内外の溶質の流出入
	rep(i, N) {
		new_con[i] += time_bunkai * p.go[i] * pow(p.size, - 1.0 / 3.0) * (prev_outside_con[i] - prev_con[i]);
		new_outside_con[i] -= time_bunkai * p.go[i] * pow(p.size, - 1.0 / 3.0) * (prev_outside_con[i] - prev_con[i]) * p.size / box_size;
	}
...
void evolve(void)
{
	...
	rep(j, N) cell[cell_number].go[j] = (j % 3 / 2) * 1;
...
int win0 = 0;
int win1 = 0;
void main_(int get, double give)
{
	//randomの種を与える
	srand(get);
...
int main(void)
{
	rep(j, 11) {
		win0 = 0;
		win1 = 0;
		rep(i, 50) main_(i + 1, (double)j * 0.1);
		take_log_winner << win0 << " " << win1 << endl;
	}
	return 0;
}
